...
/* Draw a single line, returns the y after moving down by lineHeight. */
function drawLine(page, text, x, y, font, size, color, lineHeight) {
...
async function renderDocPdf(doc) {
  ...
  /* Brand + title */
  y = PAGE_H - 60;
  drawLine(page, 'VEDRYXTECH', MARGIN_X, y, bold, 10, TEXT_5, 22);
  y -= 6;
  page.drawText(title, { x: MARGIN_X, y: y - 28, size: 32, font: bold, color: INK });
  y -= 44;
  drawLine(page, String(doc.number || ''), MARGIN_X, y, bold, 10, TEXT_5, 20);
  y -= 6;

  /* Meta row: BILLED TO on the left, DATE / DUE on the right */
  const metaTop = y - 10;
  const rightX = MARGIN_X + CONTENT_W / 2 + 30;

  /* BILLED TO */
  let ly = metaTop;
  drawLine(page, 'BILLED TO', MARGIN_X, ly, bold, 9, TEXT_5, 14);
  ly -= 2;
  drawLine(page, doc.client?.name || 'Client', MARGIN_X, ly, bold, 13, INK, 18);
  ...
  for (const line of wrappedInfo) {
    drawLine(page, line, MARGIN_X, ly, helv, 11, TEXT_3, 15);
    ly -= 15;
  }

  /* DATE + VALID UNTIL / DUE DATE */
  let ry = metaTop;
  drawLine(page, 'DATE', rightX, ry, bold, 9, TEXT_5, 14);
  ry -= 2;
  drawLine(page, fmtDate(doc.issueDate), rightX, ry, bold, 11, INK, 20);
  ry -= 8;
  drawLine(page, dateLabel, rightX, ry, bold, 9, TEXT_5, 14);
  ry -= 2;
  drawLine(page, fmtDate(doc.validUntil), rightX, ry, bold, 11, INK, 20);

  y = Math.min(ly, ry) - 20;

  /* Items table header */
  page.drawLine({
    start: { x: MARGIN_X, y: y + 8 }, end: { x: MARGIN_X + CONTENT_W, y: y + 8 },
    thickness: 1.4, color: INK,
  });
  page.drawText('DESCRIPTION', { x: MARGIN_X, y: y + 14, size: 9, font: bold, color: INK });
  const amountLabel = 'AMOUNT';
  const amountLabelW = bold.widthOfTextAtSize(amountLabel, 9);
  page.drawText(amountLabel, {
    x: MARGIN_X + CONTENT_W - amountLabelW, y: y + 14, size: 9, font: bold, color: INK,
  });
  y -= 6;
  ...
  /* Notes */
  if (doc.notes) {
    drawLine(page, 'NOTES & TERMS', MARGIN_X, y, bold, 9, TEXT_5, 14);
    y -= 4;
    for (const line of wrapLines(doc.notes, helv, 10, CONTENT_W)) {
      if (y < 60) break;
      page.drawText(safeText(line), { x: MARGIN_X, y, size: 10, font: helv, color: TEXT_2 });
      y -= 14;
    }
  }
