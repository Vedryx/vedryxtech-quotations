...
async function renderDocPdf(doc) {
  ...
  /* Brand + title. Explicit baselines throughout — no cursor arithmetic. */
  drawText(page, 'VEDRYXTECH', MARGIN_X, PAGE_H - 46, bold, 10, TEXT_5);
  drawText(page, title,        MARGIN_X, PAGE_H - 88, bold, 32, INK);
  drawText(page, String(doc.number || ''), MARGIN_X, PAGE_H - 116, bold, 10, TEXT_5);

  /* Meta row: BILLED TO on the left, DATE / DUE on the right. Both columns
     start at the same top baseline; we track their y independently and take
     the lower of the two as the cursor for the next section. */
  const metaTop = PAGE_H - 150;
  ...
  /* DATE + VALID UNTIL/DUE DATE — same rhythm as the left column. */
  let ry = metaTop;
  drawText(page, 'DATE', rightX, ry, bold, 9, TEXT_5);
  ry -= GAP_AFTER_SMALLCAPS;
  drawText(page, fmtDate(doc.issueDate), rightX, ry, bold, 11, INK);
  ry -= GAP_AFTER_VALUE;
  drawText(page, dateLabel, rightX, ry, bold, 9, TEXT_5);
  ry -= GAP_AFTER_SMALLCAPS;
  drawText(page, fmtDate(doc.validUntil), rightX, ry, bold, 11, INK);
  ry -= GAP_AFTER_VALUE;
